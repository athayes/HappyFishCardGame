...
def test_rotate_hands():
    ...
    players[0].hand = make_card_list([("H_Zero", 3)])
    players[1].hand = make_card_list([("H_One", 3)])
    players[2].hand = make_card_list([("H_Two", 3)])
    players[3].hand = make_card_list([("H_Three", 3)])
    players = rotate_hands(players)
    np.testing.assert_array_equal(players[0].hand, make_card_list([("H_One", 3)]))
    np.testing.assert_array_equal(players[1].hand, make_card_list([("H_Two", 3)]))
    np.testing.assert_array_equal(players[2].hand, make_card_list([("H_Three", 3)]))
    np.testing.assert_array_equal(players[3].hand, make_card_list([("H_Zero", 3)]))
...
def test_to_json():
    game = Game(
        ["reb", "Cool H"],
        make_card_list([("Maki", 6), ("Sashimi", 6), ("Salmon", 6)]),
        3
    )
    json = game.to_json()
    print(json)
