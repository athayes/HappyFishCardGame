...
def test_score_maki_2():
    ...
    assert players[0].score_report.report_entries[0].description == 'Maki: Second place with 2'
    assert players[0].score_report.report_entries[0].score == 3
    assert players[1].score == 6
    assert players[1].score_report.report_entries[0].description == 'Maki: First place with 4'
